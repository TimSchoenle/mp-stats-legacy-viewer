...
impl DataProvider for ClientDataProvider {
    async fn fetch_meta(&self, edition: &PlatformEdition,
        ...
        api::fetch_meta(edition).await.map_err(|e| anyhow::anyhow!(e))
    }

    async fn fetch_id_map(&self, edition: &PlatformEdition,
    ) -> Result<IdMap> {
        api::fetch_id_map(edition).await.map_err(|e| anyhow::anyhow!(e))
    }
    ...
    async fn fetch_player(&self, edition: &PlatformEdition, uuid: &str) -> Result<JavaPlayerProfile> {
    ...
    async fn fetch_game_leaderboards(&self, edition: &PlatformEdition,
                                     game_id: &str) -> Result<GameLeaderboardData> {

...
`v0.11.0`; which variable names *this* deployment spells is
...
An **unknown key inside a block fails the boot**, naming it. Every block in [§3](#3-keys) is
closed — `#[serde(deny_unknown_fields)]` — so `dist_dr` in `[server]`, or
`MP_STATS_SERVER__DIST_DR`, stops the process rather than leaving the compiled default in place
for the discrepancy to surface during an incident. A key *outside* those blocks is still
ignored, which is what lets one file carry both: the server does not fail on `[converter]`. The
keys in [§5](#5-removed-keys) are read by nothing at all — they carry no `MP_STATS_` prefix, so
no block ever sees them to refuse them. `MP_STATS_EXPLAIN=1`
...
its own CI instead of reaching a cluster. The blocks are closed
([§1](#1-how-configuration-is-loaded)), so such a key would stop the boot as well — but a
rendered release that crash-loops is a worse place to learn it than a pull request that will not
merge.
